...
def showorders():
   err=""
   qry="select A.Order_id, A.Customer_id, A.o_date,B.product_name, B.price, B.image, A.shipped, A.confirm,A.Oid,A.cancel  from myorder A  join product B  on A.product_id = B.product_id"
   data=ob.getQryData(qry)
   return render_template("dash/showorder.html",err=data['data'])

@app.route("/shippedOrder/<id>")
def shippedOrder(id):
    ob.update("myorder",{"shipped":1},"order_id",id)
    ob.update("shipping",{"ship_date":datetime.now()},"order_id",id)
...
def del_product(id,x):
    if(x=="1"):
        ob.update("myorder",{"cancel":0},"Oid",id)
    else:
        ob.update("myorder",{"cancel":1},"Oid",id)
